...
# Unmodified from old version
class ContainerNode(object):
    """
    Represents a given container in a running SMACH system.
    ...
    def update_status(self, msg):
        """
        Update the known userdata and active state set and return True if
        the graph needs to be redrawn.
        """
    ...
    def get_dotcode(self, selected_paths, closed_paths, depth, max_depth,
                    containers, show_all, label_wrapper, attrs={}):
        """
        ...
        dotstr += ContainerNode.graph_attr_string(attrs)
        ...
        dotstr += '"{}" {};\n'.format(
                ...
                ContainerNode.attr_string(proxy_attrs))
        ...
        if max_depth == -1 or depth <= max_depth:
            ...
            dotstr += ContainerNode.graph_attr_string(outcomes_attrs)
            ...
            for outcome_label in self._container_outcomes:
                ...
                dotstr += '"{}" {};\n'.format(
                    outcome_path,
                    ContainerNode.attr_string(outcome_attrs))
            dotstr += "}\n"
            ...
            for child_label in self._children:
                ...
                if child_path in containers:
                    ...
                    dotstr += '"{}" {};\n'.format(
                        child_path,
                        ContainerNode.attr_string(child_attrs))
            ...
            for (outcome_label, from_label, to_label) in internal_edges:
                ...
                if show_all \
                    ...
                    edge_attrs = {
                        'URL':      ': '.join(
                            [from_path, outcome_label, to_path]),
                        'fontsize': '12',
                        'label':    '\\n'.join(
                            label_wrapper.wrap(outcome_label))
                    }
                    ...
                    if to_label in self._container_outcomes:
                        to_key = '"{}:{}"'.format(self._path, to_label)
                        edge_attrs['color'] = '#00000055'  # '#780006'
                    else:
                        if to_path in containers:
                            edge_attrs['lhead'] = 'cluster_'+to_path
                            to_path = '/'.join([to_path, '__proxy__'])
                        to_key = '"{}"'.format(to_path)

                    dotstr += '{} -> {} {};\n'.format(
                        from_key,
                        to_key,
                        ContainerNode.attr_string(edge_attrs))

        dotstr += '}\n'
        return dotstr

    def set_styles(self, selected_paths, depth, max_depth, items,
                   subgraph_shapes, containers):
        """
        Update the styles for a list of containers without regenerating
        the dotcode.

        This function is called recursively to update an entire tree.

        @param selected_paths: A list of paths to nodes that are
                               currently selected.
        @param depth: The depth to start traversing the tree
        @param max_depth: The depth to traverse into the tree
        @param items: A dict of all the graph items, keyed by url
        @param subgraph_shapes: A dictionary of shapes from the
                                rendering engine
        ...
        if max_depth == -1 or depth <= max_depth:
            ...
            for child_label in self._children:
                ...
                active_color = ContainerNode.hex2t('#5C7600FF')
                active_fillcolor = ContainerNode.hex2t('#C0F700FF')

                initial_color = ContainerNode.hex2t('#000000FF')
                ...
                if child_path in selected_paths:
                    child_color = ContainerNode.hex2t('#FB000DFF')
    ...
    def attr_string(attrs):
        ...
        attrs_strs = ['"{}"="{}"'.format(str(k), str(v))
                      for k, v in attrs.items()]
        return ' [{}]'.format(', '.join(attrs_strs))

    @staticmethod
    def hex2t(color_str):
        """Convert a hexadecimal color strng into a color tuple."""
        color_tuple = [int(color_str[i:i + 2], 16) / 255.0
